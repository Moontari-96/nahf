...
$(function(){
	...
	// promotion 없을 때
	$('.promotion_area .swiper-button-close').on('click',function(){
		$(this).closest('.section').toggleClass('no_promotion');
	})
	...
	var gnb_box = {
		...
		action : function(){
			...
			//quick menu
			if($('.btn_quick').length){
				if($('.btn_quick').hasClass(isOpen)){
					$('.btn_quick').append('<span class="blind a11y">확장됨</span>')
				}else{
					$('.btn_quick').append('<span class="blind a11y">축소됨</span>')
				}
			}
			...
			if($('.search_area').length){
				$('.search_area .btn_s_menu').append('<span class="blind a11y">축소됨</span>')
			}
			$('.gnb_area .menu_area > li.search_area > button').on('click',function(){
				...
				if($(this).closest('.gnb_area').hasClass('search_open')=== true){
					...
					$(this).find('.a11y').text('축소됨');
					$(this).siblings('.search_box').find('.inner').stop().slideUp(400, function(){
						$('.gnb_area .menu_area > li.search_area > button').focus();
					});
				}
			});
    ...
    var snb_area = {
		...
		action : function(){
            ...
            $('.snb_area ul.dep_02 > li > a').on('click', function(){
				if($('.snb_area ul.dep_02 > li').hasClass('arrow')=== true){
					if(!($(this).closest('li').hasClass(isOpen)) && !($(this).closest('li').hasClass(isActive))){
						...
						$('.snb_area ul.dep_02 li .a11y').text('축소됨');
						$(this).find('.a11y').text('확장됨');
					} else{
			...
			//접근성
			if($('.snb_area').length){
				$('.snb_area .dep_01 > li .dep_02 > li.arrow').each(function(){				
					if($(this).hasClass('is_open')){
						$(this).find('a').append('<span class="blind a11y">확장됨</span>')
					}else{
						$(this).find('a').append('<span class="blind a11y">축소됨</span>')
					}
				});
			}
        }
	...
	//agency
	if($('.select_wrap.agency').length){
		if($('.select_wrap.agency').hasClass(isOpen)){
			$('.select_wrap.agency .btn_sel').append('<span class="blind a12y">확장됨</span>')
		}else{
			$('.select_wrap.agency .btn_sel').append('<span class="blind a12y">축소됨</span>')
		}
	}
	$('.select_wrap.agency').on('click',function(){
		if($(this).hasClass(isOpen) === true){
			$('.select_wrap.agency .btn_sel').find('.a12y').text('확장됨');
		}else{
			$('.select_wrap.agency .btn_sel').find('.a12y').text('축소됨');
		}
	});
	...
	var selectToggle = function(target){ // target은 select_wrap
        ...
        if(!target.hasClass('is_open')){
			...
			$('.select_wrap.agency .btn_sel').find('.a11y').text('확장됨');
        } else{
			target.removeClass('is_open');
            target.find('.sel_option').stop().fadeOut(100);
            target.find('a').focus();
			$('.select_wrap.agency .btn_sel').find('.a11y').text('축소됨');
        }
    }
	...
	/* ------------------- */
	// DESC 2023-02-28 sec02 zone 2 뉴스레터 -> 팝업존으로 변경에 따른 주석처리
	//sec02 뉴스레터 마지막 .btn_more 이동
	// $(".sec02 .section_con .zone02 .maintab li:last-child .tab_con .swiper-button-next").on("keydown", function(event){
	// 	if(event.shiftKey && event.keyCode == 9){
	// 		setTimeout(function(){
	// 			$(".sec02 .section_con .zone02 .maintab li:last-child .tab_con .swiper-button-pause.is_open").focus();
	// 		},100);
	// 		return false;
	// 	}
	// 	if(event.keyCode == 9 && !_shift){
		...
		if(!$(".is_main").hasClass('language')){
			if($(window).innerWidth() >= 750){
				if(!($('.sec02 .section_con .zone02 .maintab > li:last-child').hasClass('is_open'))){
					...
					if(event.shiftKey && event.keyCode == 9){
							if($('.sec02 .section_con .zone02 .maintab > li:nth-child(2)').hasClass('is_open')){
							sec02LiShiftTab = setTimeout(function(){
								$(".sec02 .section_con .zone02 .maintab li:nth-child(2) .tab_con .newsletter_area > .btn_more").focus();
							},100);
							return false;
						}
					}
					...
					$(".sec02 .section_con .zone02 .maintab > li .popzone_area .popzone_indicator .swiper-button-next").on("keydown", function(event){
						if(event.shiftKey && event.keyCode == 9){
							sec02LiShiftTab = setTimeout(function(){
								$(".sec02 .section_con .zone02 .maintab > li .popzone_area .popzone_indicator .ind_middle button.is_open").focus();
							},300);
							return false;
						}
						if(event.keyCode == 9 && !_shift){
	...
	// 				if ($('.sec02 .section_con .zone02 .maintab > li:nth-child(2)').hasClass('is_open')) {
	// 					sec02LiShiftTab = setTimeout(function(){
	// 						$(".sec02 .section_con .zone02 .maintab li:nth-child(2) .tab_con a.btn_more").focus();
	// 					},100);
	// 					return false;
	// 				} else {
	// 					sec02LiShiftTab = setTimeout(function(){
	...
	$(".sec03 .policy_box:first-child > a.move_list").on("keydown", function(event){
		if($(".sec02 .section_con .zone02 .maintab li:last-child").hasClass("is_open")){
			if(event.shiftKey && event.keyCode == 9){
				...
				$("#fp-nav > ul > li:eq(1) a").trigger("click");
				setTimeout(function(){
					$(".sec02 .section_con .zone02 .maintab li:last-child .tab_con .swiper-button-next").focus();
				},300);
				return false;
			}
