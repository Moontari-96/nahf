...
$(function(){
	...
	// sec02 공통
	var acco_txt = $('<em class="blind">축소됨</em>');
	$('.section_con .maintab_area .maintab > li .tab_top .tab_tit').append(acco_txt);
	$('.section_con .maintab_area .maintab > li.is_open .tab_top .tab_tit').find('em.blind').text("확장됨");
	$('.section_con .maintab_area .maintab > li .tab_top .tab_tit').on('click',function(){
		...
		if($(this).closest('li').hasClass(isOpen)){
			$(this).find('em.blind').text("확장됨");
			$(this).closest('li').siblings().find('em.blind').text("축소됨");
		}
	});
	...
	var gnb_box = {
		...
		action : function(){
			...
			//btn_share -> addthis로 수정
			if($('.btn_share').length){
				$('.btn_share').append('<span class="blind a11y">축소됨</span>')
			}
	...
	$('.select_wrap.pc_lang .sel_option li:last-child a').on('keydown',function(event){
		if(event.shiftKey && event.keyCode == 9){
			setTimeout(function(){
				$('.select_wrap.pc_lang .sel_option li:nth-child(3) a').focus();
			},100);
			return false;
		}
		if(event.keyCode == 9 && !_shift){
	...
	$('li.is_open > .toggle_caption > .title').children().find('span.blind').text("확장됨");
	...
	$('.toggle_caption .title').on('click',function(){
		...
		if($(this).closest('li').hasClass(isOpen)){
			$(this).children().find('span.blind').text("확장됨");
		}else{
	...
	$(".gnb_area .menu_area > li.btn_mobmenu_box .btn_mobmenu").on("keydown", function(event){
		if(event.keyCode == 13){
			setTimeout(function(){
				$(".header_wrap.mobmenu_open .link_area .link_list02 > li.lang_box .language").focus();
			},100);
		}
	})
	// gnb 모바일 tab 이동 
	$(".gnb_area .menu_area > li.btn_mobmenu_box .btn_mobmenu").on("click", function(){
		$(".header_wrap.mobmenu_open .gnb_area .menu_area > li.btn_mobmenu_box > button.btn_close").on("keydown", function(event){
			if(event.shiftKey && event.keyCode == 9){
				setTimeout(function(){
					if($(".gnb_area .gnb_box > .dep_01 > li:last-child .dep_02_wrap").css('display') == 'block'){
						$(".gnb_area .gnb_box > .dep_01 > li:last-child a").last().focus();
					} else{
						$(".gnb_area .gnb_box > .dep_01 > li:last-child > a").focus();
					}
				},100);
				return false;
			}
		...
		$(".header_wrap.mobmenu_open .link_area .link_list02 > li.lang_box .language").on("keydown", function(event){
			if(event.shiftKey && event.keyCode == 9){
				setTimeout(function(){
			...
			if(event.keyCode == 9 && !_shift){
				setTimeout(function(){
					$(".gnb_area .gnb_box > .dep_01 > li:first-child a").focus();
				},100);
				return false;
			}
		})
		$(".gnb_area .gnb_box > .dep_01 > li:first-child a").on("keydown", function(event){
			if(event.shiftKey && event.keyCode == 9){
				setTimeout(function(){
					$(".header_wrap.mobmenu_open .link_area .link_list02 > li.lang_box .language").focus();
				},100);
	...
	/* ------------------- */
	// sec01 홍보영역있을때와 없을때 탭 이동
	$(".is_main .sec01:not(.no_promotion) .promotion-swiper a.img_box").on("keydown", function(event){
		console.log("_shift: ");
		if(event.shiftKey && event.keyCode == 9){
			setTimeout(function(){
	...
	$(".is_main .sec01.no_promotion .visual_indicator .ind_middle button.is_open").on("keydown", function(event){
		console.log("_shift: ");
		if(event.shiftKey && event.keyCode == 9){
			setTimeout(function(){
				$(".is_main .sec01 .swiper-button-next").focus();
	...
	//sec02 shift + tab sec01로 이동 
	$(".sec02 .section_con .zone01 .maintab li.is_open .tab_top > a").on("keydown", function(event){
		if(event.shiftKey && event.keyCode == 9){
			$("#fp-nav > ul > li:eq(0) a").trigger("click");
			if($('.is_main .sec01').hasClass("no_promotion")){
				setTimeout(function(){
					$(".is_main .sec01.no_promotion .visual_indicator .ind_middle button.is_open").focus();
				},300);
				return false;
			}else if(!$('.is_main .sec01').hasClass("no_promotion")){
				setTimeout(function(){
	...
	$(".sec02 .section_con .zone02 .maintab li:last-child .tab_con .btn_more").on("keydown", function(event){
		if(event.shiftKey && event.keyCode == 9){
			setTimeout(function(){
				$(".sec02 .section_con .zone02 .maintab li:last-child .tab_con a.txt_box").focus();
			},100);
			return false;
		}
		if(event.keyCode == 9 && !_shift){
			...
			setTimeout(function(){
				$(".sec03 .policy_box:first-child > a.move_list").focus();
			},300);
			return false;
		}
	});
		...
		if(!$(".is_main").hasClass('language')){
			if($(window).innerWidth() >= 750){
				if(!($('.sec02 .section_con .zone02 .maintab > li:last-child').hasClass('is_open'))){
					...
					if(event.keyCode == 9 && !_shift){
						...
						sec02LiTab = setTimeout(function(){
							$(".sec02 .section_con .zone02 .maintab li:last-child .tab_con a.img_box").focus();
						},300);
						return false;
					}
				}
			...
			if(!($('.sec02 .section_con .zone02 .maintab > li:last-child').hasClass('is_open'))){
				...
				if(event.keyCode == 9 && !_shift){
					$("#fp-nav > ul > li:eq(2) a").trigger("click");
					sec02LiTab = setTimeout(function(){
						$(".sec03 .publication_con .public_area .public-swiper ul > li:first-child > a.move_list").focus();
					},300);
					return false;
				}
			}else{
	...
	//sec03 shift + tab sec02로 이동
	$(".sec03 .policy_box:first-child > a.move_list").on("keydown", function(event){
		if($(".sec02 .section_con .zone02 .maintab li:last-child").hasClass("is_open")){
			if(event.shiftKey && event.keyCode == 9){
				console.log("shifttab")
				$("#fp-nav > ul > li:eq(1) a").trigger("click");
				setTimeout(function(){
					$(".sec02 .section_con .zone02 .maintab li:last-child .tab_con a.btn_more").focus();
				},300);
				return false;
			}
		}else if(!$(".sec02 .section_con .zone02 .maintab li:last-child").hasClass("is_open")){
			if(event.shiftKey && event.keyCode == 9){
				console.log("shifttab")
				$("#fp-nav > ul > li:eq(1) a").trigger("click");
				setTimeout(function(){
	...
	// 다국어 shift + tab sec02로 이동
	$(".is_main.language .sec03 .publication_con .public_area .public-swiper ul > li:first-child > a.move_list").on("keydown", function(event){
		if($(".sec02 .section_con .zone02 .maintab li:last-child").hasClass("is_open")){
			if(event.shiftKey && event.keyCode == 9){
				console.log("shifttab")
				$("#fp-nav > ul > li:eq(1) a").trigger("click");
	...
	//sec03 tab 이동
	$(".sec03 .publication_con .public_area .public_slide:last-child a.move_detail").on("keydown", function(event){
		if(event.shiftKey && event.keyCode == 9){
			setTimeout(function(){
				$(".sec03 .publication_con .public_area .public_slide:last-child a.move_list").focus();
			},300);
			return false;
		}
		if(event.keyCode == 9 && !_shift){
	...
	//다국어 sec03 tab 이동
	$(".is_main.language .sec03 .publication_con .public_area .public-swiper ul > li:last-child > a.move_detail").on("keydown", function(event){
		if(event.shiftKey && event.keyCode == 9){
			setTimeout(function(){
				$(".is_main.language .sec03 .publication_con .public_area .public-swiper ul > li:last-child > a.move_list").focus();
			},300);
			return false;
		}
	});
	...
	$(".sec04 .business_area .swiper-slide:first-child > .txt_box > a").on("keydown", function(event){
		if(event.shiftKey && event.keyCode == 9){
			$("#fp-nav > ul > li:eq(2) a").trigger("click");
			setTimeout(function(){
				$(".sec03 .publication_con .public_area .public_slide:last-child a.move_detail").focus();
			},300);
			return false;
		}
	});
	...
	/* addthis 접근성 작성중 */
	if($('.addthis_list').length){
		function addthisReady(evt) {
			//link a링크 enter 안되어 임시버튼을 a링크(addthis a링크) 상위에 놓고 강제 click으로 처리하려 했으나 동적인 요소 trigger click되지 않음
			//임시버튼을 a링크(addthis a링크) 상위에 동적 생성
			$('.addthis_list').append( '<a href="javascript:;" class="btn_url"><span class="blind">Copy Link</span></a>');
			setTimeout(
				function(){
					$('.share_box .addthis_list .at-share-tbx-element .at-share-btn.at-svc-link').attr('tabindex','-1');
					$('.share_box .addthis_list .at-share-tbx-element .at-share-btn.at-svc-print').attr('title','새창열림');
					console.log('setTimeout')
				},1000);
			$(document).on('keyup','.con_wrap .tit_area .share_box > div > a.btn_url',function(event){
				if(event.keyCode==13) { // enter 이벤트
					console.log('keyup');
					// $('.share_box .addthis_list .at-share-tbx-element .at-share-btn.at-svc-link')[0].click();
					$('.share_box .addthis_list .at-share-tbx-element .at-share-btn.at-svc-link').css('background','purple').trigger('click');
		...
		//.share_box 새창열림 부여
		$(document).one('click','.share_box .btn_share',function(){
			$(this).siblings('.addthis_inline_share_toolbox').find('.at-share-tbx-element .at-share-btn:not(.at-svc-compact)').attr('title','새창열림')
		});

		//링크버튼 클릭시 레이어팝업 안에 가짜 copy버튼 생성및 enter 스크립트
		$(document).one('click','.share_box .addthis_list .at-share-tbx-element .at-share-btn.at-svc-link',function(){
			$('#at-expanded-menu-host .at-copy-link-share-button.at-expanded-menu-primary-action-btn').replaceWith( '<button class="at-copy-link-share-button at-expanded-menu-primary-action-btn btn_new">Copy</button>');
		});
		$(document).on('keyup','#at-expanded-menu-host .at-expanded-menu-primary-action-btn.btn_new',function(event){
			if(event.keyCode==13) { // enter 이벤트
				$('#at-expanded-menu-host .at-expanded-menu-primary-action-btn:not(.bnt_new)').trigger('click')
			}
		});

		//addthis 레이어팝업 접근성
		$(document).on('keydown','#at-expanded-menu-host .at-branding-logo.at-expanded-menu-branding',function(event){
			if(event.shiftKey && event.keyCode == 9){
				setTimeout(function(){
					$('#at-expanded-menu-host .at-copy-link-share-button').focus();
				},200);
				return false;
			}
			if(event.keyCode == 9 && !_shift){
				setTimeout(function(){
					$('#at-expanded-menu-host .at-expanded-menu-close').focus();
				},200);
				return false;
			}
		});
		$(document).on('keydown','#at-expanded-menu-host .at-expanded-menu-close',function(event){
			if(event.shiftKey && event.keyCode == 9){
				setTimeout(function(){
					$('#at-expanded-menu-host .at-branding-logo.at-expanded-menu-branding').focus();
				},200);
				return false;
			}
			if(event.keyCode == 9 && !_shift){
				setTimeout(function(){
					$('#at-expanded-menu-host .at-copy-link-share-page-url').focus();
				},200);
				return false;
			}
		});
		$(document).on('click','#at-expanded-menu-host .at-expanded-menu-close',function(){
			$(".at-svc-link").focus();
		})
		$(document).on("keydown",".at-svc-compact", function(event){
			if(event.keyCode == 13 && !_shift){
				setTimeout(function(){
					$('#at-expanded-menu-service-filter').focus();
				},200);
				$(document).on("keydown",".at-expanded-menu-close", function(event){
					if(event.shiftKey && event.keyCode == 9){
						setTimeout(function(){
				...
				$(document).on("keydown",".at-branding-logo", function(event){
					if(event.shiftKey && event.keyCode == 9){
						setTimeout(function(){
							$('.at-expanded-menu-load').focus();
						},200);
						return false;
					}
					if(event.keyCode == 9 && !_shift){
				...
				$(document).one("click",".at-expanded-menu-close", function(){
